...
public:
    ...
    Graph(int V) : vertexHead(nullptr), V(V) {
        ...
        for (int i = 0; i < V; i++) {
            ...
            if (!vertexHead) vertexHead = newVertex;
            else prev->next = newVertex;
            prev = newVertex;
        }
    }
...
void dijkstra(Graph& g, int src) {
    ...
    for (int count = 0; count < V; count++) {
        ...
        int minDist = INT_MAX;
        int u = -1;
        for (int i = 0; i < V; i++) { // vertex i hasn't been visited yet, AND it's closer than the best candidate found so far
